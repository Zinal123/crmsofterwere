    ...
    public function getDataTableResponse(Request $request): array
    {
        ...
        $data = $rows->map(function ($item) use ($canBulkDelete) {
            if ($item->amount == $item->paidamount) {
                ...
                $paymentButton = '<button type="button" class="btn btn-soft-primary btn-sm open-modal" data-id="' . e($item->id) . '" data-customer="' . e($item->customer_id) . '"data-bs-toggle="modal" data-bs-target="#exampleModalgrid" title="Payment" aria-label="Payment"><i class="ri-secure-payment-line align-bottom"></i></button>';
            }

            $actionHtml = '<div class="d-flex gap-2 flex-wrap">'
                . '<div class="edit"><a href="' . route('invoice.details', $item->id) . '"><button class="btn btn-soft-success btn-sm edit-item-btn" title="Details" aria-label="Details"><i class="ri-eye-line align-bottom"></i></button></a></div>'
