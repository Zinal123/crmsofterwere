...
- **`<x-ui.data-table-card>`** — standardized card + table + toolbar shell (search box position, Create button position, consistent table classes). Used by every list page. **Correction from initial audit**: only Invoices has genuine server-side DataTables (`serverSide: true` + AJAX endpoint); Inventory/Vendor/Quotation/Payment-History either have no working init or fall back to Velzon's untouched demo boilerplate, working only because the full dataset is already dumped into the page HTML and DataTables re-paginates it client-side. This phase standardizes the *visual shell and client-side behavior* (search box position, empty/loading state styling) across all of them without changing how each page gets its data — real server-side pagination for the 4 pages that lack it is explicitly deferred to a future backend task, not part of this UI/UX pass (see Rollout, Phase 6, and Out of scope).
...
6. **Table visual consolidation** — wrap every list page (Invoices, Inventory, Vendor, Quotation, Payment History, and the plain-table admin pages Roles/Users, and the `list.js`-based Product page) in the same `<x-ui.data-table-card>` shell: consistent search-box position, consistent empty/loading-state styling, consistent table classes. This phase is visual/client-side only — it does **not** add real server-side pagination to the 4 modules that currently lack it (Inventory/Vendor/Quotation/Payment-History), since that's genuine backend work (a `listData()`-style controller method + AJAX endpoint per module, mirroring what Invoices already has), bigger in scope and risk than a UI/UX pass, and explicitly deferred — see Out of scope. Admin Roles/Users do gain real client-side DataTables (search/sort/pagination) as part of this phase, since that's a client-side-only addition (no new backend endpoint needed, their full dataset is already small and server-rendered) and directly fixes their current complete absence of search/sort.
...
- Real server-side pagination for Inventory, Vendor, Quotation, and Payment History (a `listData()`-style controller + AJAX endpoint per module, matching what Invoices already has) — discovered during implementation planning that these modules currently have no real server-side DataTables despite appearing to. Genuine backend work, bigger in scope/risk than a UI/UX pass; deferred to a future task. This spec's Phase 6 only standardizes their visual shell and client-side behavior.
